...
import { predictDogBreed } from '../../services/dogBreedService';
...
const StorybookScreen = ({navigation, route}: any) => {
  ...
  const handleSetTitleImage = async (imageUri: string) => {
    setTitleImage(imageUri);
    try {
      const result = await predictDogBreed(imageUri);
      if (result && result.confidence >= 0.5) {
        setTags(prev => prev.includes(result.breed) ? prev : [...prev, result.breed]);
      }
    } catch (e) {
      // 에러 핸들링 (옵션)
    }
                  ...
                  {block.type === 'Text' ? (
                      ...
                      <View>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => handleSetTitleImage(block.value)}>
