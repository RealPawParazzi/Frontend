...
const transformData = () => {
    const updatedPetList = petStore.getState().pets.map((pet) => ({
        ...
        name: pet.name,
        species: pet.type,
        image: pet.petImg ? { uri: String(pet.petImg) } : require('../assets/images/pets-1.jpg'),
    }));

    const updatedRecentPosts = boardStore.getState().boardList.map((board) => ({
        id: String(board.id),
        title: board.title,
        image: board.titleImage ? { uri: String(board.titleImage) } : require('../assets/images/post-1.jpeg'),
    }));
