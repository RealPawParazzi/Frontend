 ...
 * ✅ 반려동물 등록 API
 * @param userId 회원 ID
 * @param data 반려동물 등록 데이터 (이름, 종, 나이)
 * @returns 등록된 반려동물 객체 반환
 * @throws 반려동물 등록 실패 시 오류 발생
 */
export const registerPet = async (userId: number, data: { name: string; breed: string; age: number }) => {
 ...
 * ✅ 반려동물 정보 수정 API
 * @param petId 반려동물 ID
 * @param data 수정할 반려동물 정보 (이름, 종, 나이)
 * @returns 수정된 반려동물 객체 반환
 * @throws 반려동물 정보 수정 실패 시 오류 발생
 */
export const updatePet = async (petId: number, data: { name?: string; breed?: string; age?: number }) => {
    const response = await fetch(`${API_BASE_URL}/${petId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
